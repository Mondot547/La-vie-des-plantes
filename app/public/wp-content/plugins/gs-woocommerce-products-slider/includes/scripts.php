...
final class Scripts {
	...
	public function add_assets() {
		...
		// Styles
		$this->add_style( 'gs-bootstrap-grid', GSWPS_FILES_URI . '/assets/libs/bootstrap-grid/bootstrap-grid.min.css', [], GSWPS_VERSION );
		$this->add_style( 'gs-swiper', GSWPS_FILES_URI . '/assets/libs/swiper-js/swiper.min.css', [], GSWPS_VERSION );
		
		// Scripts
		$this->add_script( 'gs-swiper', GSWPS_FILES_URI . '/assets/libs/swiper-js/swiper.min.js', ['jquery'], GSWPS_VERSION, true );
		
		if ( ! is_gswps_pro() ) {
			$this->add_style( 'gswps-public', GSWPS_FILES_URI . '/assets/css/public.min.css', [], GSWPS_VERSION );
			$this->add_script( 'gswps-public', GSWPS_FILES_URI . '/assets/js/public.min.js', [ 'jquery' ], GSWPS_VERSION, true );
		}

		do_action( 'gswps__add_assets', $this );
	}
	...
	public function _get_public_style_all() {
		return (array) apply_filters( 'gs_woo_slider_get_public_style_all', [
			'gs-bootstrap-grid',
			'gs-swiper',
			'gswps-public'
		]);
	 ...
	 */
	//need to be fixed public scripts warning
	public function _get_public_script_all() {
		return (array) apply_filters( 'gs_woo_slider_get_public_script_all', [
			'gs-swiper',
			'gswps-public'
		]);
	}
	...
	public function _get_admin_style_all() {
		return (array) apply_filters( 'gs_woo_slider_get_admin_style_all', []);
	}
	...
	public function _get_admin_script_all() {		
		return (array) apply_filters( 'gs_woo_slider_get_admin_script_all', []);
	}
	...
	public function _get_assets_all( $asset_type, $group, $excludes = [] ) {
		...
		return (array) apply_filters( sprintf( 'gs_woo_slider_%s__%s_all', $group, $asset_type ), $assets );
	}
	...
	public function print_plugin_icon_css() {
		echo "<style>#adminmenu .toplevel_page_gs-woo-slider .wp-menu-image img, #adminmenu .menu-icon-gs-woo-slider .wp-menu-image img{padding-top:7px;width:20px;opacity:.8;height:auto;}</style>";
	}
	...
	public function enqueue_scripts( $hook ) {
		...
		gsWooAssetGenerator()->enqueue( gsWooAssetGenerator()->get_current_page_id() );

		do_action( 'gs_woo_slider_assets_loaded' );			
	}
	...
	public function enqueue_admin_script( $hook ) {
		...
		// Allow scripts loading in new gs_woo_slider member page
		if ( 'toplevel_page_gs-woo-slider' === $hook && $_GET['page'] === 'gs-woo-slider' ) $load_script = true;
